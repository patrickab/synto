"""
Tests for metrics.LLMCallEvent emission from request_structured.
...
def test_no_sink_no_emission():
    """Outside metrics_sink(), request_structured must not crash and emit nothing."""
...
def test_final_failure_emits_tier_minus_one():
    ...
    with metrics_sink() as events, pytest.raises(StructuredOutputError):
        request_structured(
            client=c,
            prompt="p",
            model_class=AnalysisResult,
...
def test_stage_is_passthrough():
    ...
    assert events[0].model_role == "heavy"
