...
def test_sanitize_obsidian_math_converts_display_math_before_link_repairs():
    from synto.markdown_math import sanitize_obsidian_math
...
def test_sanitize_obsidian_math_wraps_standalone_bare_latex_line():
    from synto.markdown_math import sanitize_obsidian_math
...
def test_sanitize_obsidian_math_keeps_code_and_existing_math_unchanged():
    from synto.markdown_math import sanitize_obsidian_math
...
def test_build_annotations_low_confidence(db):
    annotations = _build_draft_annotations(0.3, ["raw/a.md"], db)
    assert any("low-confidence" in a for a in annotations)


def test_build_annotations_single_source(db):
    annotations = _build_draft_annotations(0.8, ["raw/a.md"], db)
...
def test_build_no_annotations_above_threshold(db):
    ...
    db.upsert_raw(rec_a)
    db.upsert_raw(rec_b)
    annotations = _build_draft_annotations(0.75, ["raw/a.md", "raw/b.md"], db)
...
def test_build_annotations_all_low_quality(db):
    ...
    db.upsert_raw(rec_a)
    db.upsert_raw(rec_b)
    annotations = _build_draft_annotations(0.6, ["raw/a.md", "raw/b.md"], db)
    assert any("low-quality" in a for a in annotations)


def test_build_annotations_prompt_degraded(db):
    annotations = _build_draft_annotations(0.75, ["raw/a.md", "raw/b.md"], db, prompt_degraded=True)
    assert any("prompt degraded to fit provider context" in a for a in annotations)


def test_strip_annotations_removes_synto_auto(db):
    body = "<!-- synto-auto: low-confidence -->\n\n## Overview\n\nContent."
    stripped = _strip_draft_annotations(body)
    assert "synto-auto" not in stripped
    assert "## Overview" in stripped
    assert "Content." in stripped


def test_strip_annotations_leaves_user_html_comments(db):
    body = "<!-- user comment -->\n\n<!-- synto-auto: single-source -->\n\nContent."
    stripped = _strip_draft_annotations(body)
    assert "<!-- user comment -->" in stripped
    assert "synto-auto" not in stripped


def test_strip_annotations_removes_legacy_olw_auto(db):
...
def test_strip_annotations_no_annotations_unchanged(db):
    body = "## Title\n\nNo annotations here."
    assert _strip_draft_annotations(body) == body
...
def test_repair_bare_bracket_links_converts_llm_slips():
    body = _repair_bare_bracket_links(
        "See [API] and [Product Backlog].", ["API", "Product Backlog"]
    )
...
def test_repair_bare_bracket_links_leaves_unknown_bracketed_prose_plain():
    body = _repair_bare_bracket_links("Known [API]. Unknown [Agile Development] note.", ["API"])
...
def test_strip_unknown_wikilinks_keeps_source_links():
    body = _strip_unknown_wikilinks(
        "Claim ([[sources/Reference Note|S1]]) and [[sources/Foo]].",
        [],
    )

    assert body == "Claim ([[sources/Reference Note|S1]]) and [[sources/Foo]]."
...
def test_annotations_stripped_on_approve(config, db, tmp_path):
    ...
    annotated_body = "<!-- synto-auto: low-confidence (0.25) -->\n\n## Overview\n\nContent."
    meta = {
        "title": "Test Article",
        "status": "draft",
        "tags": [],
...
def test_compile_concepts_preserves_existing_tags_when_model_returns_none(config, db):
    ...
    (config.wiki_dir / "Template Catalog.md").write_text(existing_note)
    db.upsert_article(
        WikiArticleRecord(
            path="wiki/Template Catalog.md",
            title="Template Catalog",
            sources=["raw/a.md"],
    ...
    client = make_mock_client(
        json.dumps({"title": "Template Catalog", "content": "Updated content.", "tags": []})
    )

    drafts, failed, _ = compile_concepts(config, client, db, concepts=["Template Catalog"])

    assert failed == []
    assert len(drafts) == 1
    draft_text = drafts[0].read_text(encoding="utf-8")
    assert "tags:\n- templates\n- catalog" in draft_text
...
def test_compile_concepts_retry_value_error_is_counted_as_context_too_large(
    ...
    from synto.openai_compat_client import LLMBadRequestError
    from synto.pipeline import compile as compile_module
